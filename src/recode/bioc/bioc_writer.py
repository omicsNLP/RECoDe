...
class BioCWriter:
    ...
    def _build_infons(self, infons_dict, infons_parent_elem):
        ...
        if type(infons_dict) != dict:
            infons_dict = dict(infons_dict)
    ...
    def _build_passages(self, passages_list, document_parent_elem):
        for passage in passages_list:
            ...
            if type(passage.offset) == int:
                _passage_offset = str(passage.offset)
            passage_elem.xpath("offset")[0].text = _passage_offset
            ...
            if len(passage.sentences) > 0:
                ...
                self._build_annotations(passage.annotations, passage_elem)
            # relation*
    ...
    def _build_annotations(self, annotations_list, annotations_parent_elem):
        for annotation in annotations_list:
            ...
            annotation_elem = annotations_parent_elem.xpath("annotation")[-1]
            # infon*
    ...
    def _build_sentences(self, sentences_list, passage_parent_elem):
        for sentence in sentences_list:
            ...
            # import IPython; IPython.embed(colors="Linux"); exit(1)
            _sentence_infons = {"offset": str(sentence.offset), "text": sentence.text}
            # infon*
            # self._build_infons(sentence.infons, sentence_elem)
            self._build_infons(_sentence_infons, sentence_elem)
            # offset
            sentence_elem.append(E("offset"))
            offset_elem = sentence_elem.xpath("offset")[0]
            offset_elem.text = str(sentence.offset)
            # text?
